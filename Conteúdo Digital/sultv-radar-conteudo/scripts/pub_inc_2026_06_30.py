...
com estado persistente, de forma idempotente:
  ...
  - Posts FB/IG: NÃO têm dedup nativo. Este driver rastreia em
    state/social_posted_2026-06-14.json (id_hash -> resultado) e pula o que já saiu.
...
def _materias_objs():
    ...
    # Carrega aprovadas para preencher campos base obrigatórios
    aprov_raw = _load_json(ROOT / "state" / f"aprovadas_{HOJE}.json", {})
    aprov_items = aprov_raw if isinstance(aprov_raw, list) else aprov_raw.get("aprovadas", aprov_raw.get("items", []))
    aprov_by_hash = {it["id_hash"]: it for it in aprov_items}
    objs = []
    defaults = {
        "coletado_em": "", "bloco": "", "fonte_id": 0,
        "resumo": "", "url": "", "publicado_em": "", "score_fonte": 0,
        "tags_secundarias": [], "justificativa": "", "alerta_guardrail": False,
        "guardrail_motivo": "", "fontes_relacionadas": [], "subtitulo": "",
        "ganchos_3": [], "angulo_editorial": "", "fontes_complementares_sugeridas": [],
        "roteiro_short_60s": {}, "lead_materia_longa": "", "card_carrossel": {},
        "tag_thumbnail": "", "chamada_faixa": "", "decisao_motivo": "", "briefing_visual": {},
    }
    for d in pauta:
        merged = dict(defaults)
        base = aprov_by_hash.get(d.get("id_hash", ""), {})
        merged.update({k: v for k, v in base.items() if k in valid})
        merged.update({k: v for k, v in d.items() if k in valid})
        objs.append(MateriaPronta(**{k: v for k, v in merged.items() if k in valid}))
    return objs
...
def cmd_sheets():
    """Fase 6b — grava pauta do dia no Google Sheets (Radar_Diario + Radar_Historico)."""
    import json as _json
    from types import SimpleNamespace
    try:
        from sheets_writer import materia_to_row, COLUNAS, _spreadsheet
    except Exception as e:
        print(f"[sheets] import falhou: {e}"); return

    STATE_DIR = ROOT / "state"
    DATA = HOJE
    pauta_path = STATE_DIR / f"pauta_{DATA}.json"
    if not pauta_path.exists():
        print(f"[sheets] pauta não encontrada: {pauta_path}"); return

    with open(pauta_path) as f:
        raw = _json.load(f)
    pauta_list = raw["pauta"] if isinstance(raw, dict) else raw

    url_map = {}
    url_path = STATE_DIR / f"url_por_hash_{DATA}.json"
    if url_path.exists():
        with open(url_path) as f:
            url_map = _json.load(f)

    objetos = []
    for item in pauta_list:
        h = item.get("id_hash", "")
        if h in url_map:
            item["url"] = url_map[h]
        item.setdefault("url", item.get("url_publicada", ""))
        item.setdefault("publicado_em", "")
        item.setdefault("score_combinado", item.get("score", 0))
        item.setdefault("bloco", "")
        item.setdefault("fonte_nome", item.get("fonte", ""))
        item.setdefault("cidade", "")
        item.setdefault("titulo", item.get("titulo_sultv", item.get("titulo_original", "")))
        item.setdefault("roteiro_short_60s", {})
        item.setdefault("post_instagram", {})
        item.setdefault("card_carrossel", {})
        objetos.append(SimpleNamespace(**item))

    try:
        ss = _spreadsheet()
        ws_names = [w.title for w in ss.worksheets()]
        ws = ss.worksheet("Radar_Diario") if "Radar_Diario" in ws_names \
             else ss.add_worksheet("Radar_Diario", rows=200, cols=len(COLUNAS))
        ws.clear()
        ws.update([COLUNAS] + [materia_to_row(m, DATA) for m in objetos])
        print(f"[sheets] ✅ Radar_Diario: {len(objetos)} linhas")
        pub = [m for m in objetos if getattr(m, "decisao_final", "") == "PUBLICAR"]
        ws_names = [w.title for w in ss.worksheets()]
        wsh = ss.worksheet("Radar_Historico") if "Radar_Historico" in ws_names \
              else ss.add_worksheet("Radar_Historico", rows=10000, cols=len(COLUNAS))
        if pub:
            wsh.append_rows([materia_to_row(m, DATA) for m in pub], value_input_option="RAW")
            print(f"[sheets] ✅ Radar_Historico: +{len(pub)} linhas PUBLICAR")
    except Exception as e:
        print(f"[sheets] ❌ {e}")
